...
class BgManager {
    ...
    init() {
        ...
        let background3 = Background(imageName: "jungleBg3", zposOffset: 0.5, isInfScroll: true, baseScrollSpeed: 20)
        entities.insert(background3)
        let background4 = Background(imageName: "jungleBg4", zposOffset: 1.5, isInfScroll: true, baseScrollSpeed: 20)
        entities.insert(background4)
        let background5 = Background(imageName: "jungleBg5", zposOffset: 5, isInfScroll: true, baseScrollSpeed: 20)
        entities.insert(background5)
    }
    
    func addEntities(to scene: SKScene) {
        for entity in entities {
            if let backgroundNode = entity.component(ofType: NodeComponent.self)?.node {
